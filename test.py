...
if str(response.status_code) in "200":
    print("App is running at "+str(host)+":"+str(port) +" and response status code is 200")
else:
    sys.exit("Something went terribly wrong. Request code:" + str(response.status_code))

print("\nDeleting marathon app "+ newId)
response = requests.delete("https://"+master+"/service/marathon/v2/apps/"+newId, cookies=authCookie, verify=False)
print("Response: "+ str(response.status_code))
